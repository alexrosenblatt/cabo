...
class Game:
    def __init__(
        ...
        self.is_open_hand: bool = (
            open_hand  # set this to see every card in each hand each round
        )
    ...
    def initialize_game(self) -> None:
        ...
        (human_player_count, computer_player_count) = present_game_start_prompt()
    ...
    def start_turn(self, player: Player):
        ...
        discard_card_swapped = self.evaluate_and_action_discard_card(
            current_player_type, current_players_pile, drawn_discard_card, player
        )
        if not discard_card_swapped:
            turn_action_choice = self.evaluate_drawn_card(player, current_player_type)
            try:
    ...
    def use_power_on_card(self, current_player_type, current_players_pile, player):
    ...
    def get_opponent_choice(self, current_player_type, current_players_pile, player):
        ...
        if len(player_list) > 2:
            if current_player_type == 0:
                ...
                return player.ai.get_computer_opponent_choice_for_power(player_list)
        else:
    ...
    def evaluate_drawn_card(self, player: Player, current_players_type):
        ...
        if current_players_type == 0:
            ...
            turn_action_choice = player.ai.get_computer_action_choice(drawn_card)
        return turn_action_choice

    def use_power(
        self,
        drawn_card_power: int,
        current_player_pile: Pile,
        target_player_pile: Pile,
        current_player_type,
        player,
    ) -> bool:
    ...
    def get_computer_discard_evaluation(self, top_discard_card, current_players_pile):
        ...
        def computer_discard_calculation():
            ...
            if discard_card_value > 0 and memorized_card_value > 0:
                threshold = float(
                    (memorized_card_value - discard_card_value) / memorized_card_value
                )
                logging.info(
    ...
    def evaluate_card_slap(self, discarded_card: Card, memorized_card: Card):
        if discarded_card.value == memorized_card.value:
            return True
        else:
            return False
    ...
    async def slap_evaluation_loop(self, discarded_card):
        ...
        for player in self.player_list:
            if player.type == 0:
                ...
                aws += asyncio.create_task(self.human_slap_evaluate(discarded_card))
            else:
                pass
    ...
    async def ai_slap_evaluate(self, card):
        return True

    async def human_slap_evaluate(self, card):
        response = get_human_slap_input()
        if response == str(7):
            return print("human_true")
        else:
            return print("human_false")
        return True
...
def swap(
    ...
    insert_and_memorize(
        destination_pile, dest_index, swap_card1, destination_pile.owner, is_blind
    )
    insert_and_memorize(
        source_pile, source_index, swap_card1, destination_pile.owner, is_blind
    )
    log_swap_results(
        source_pile, source_index, destination_pile, dest_index, swap_card1, swap_card2
    )
    return swap_card1, swap_card2


def insert_and_memorize(
    pile, index, object: Card, player_taking_action, is_public=False, is_blind=False
):
    pile.insert(index, object)
    if is_blind:
        player_taking_action.memory.insert(
            0,
            {
    ...
    if not is_public:
        present_other_player_card_discard(pile, index, object, player_taking_action)
    else:
        pass
